...
exports.EngineRenderer = void 0;
const PIXI = __importStar(require("pixi.js"));
const Textures_1 = require("./services/Textures");
class EngineRenderer {
    constructor() {
        ...
        this.createCardWithValue = (x, y, cardInfo) => {
            return this.createPlayerCard(x, y, this.getValueCode(cardInfo));
        };
        this.textures = new Textures_1.Textures();
    }
    setApp(app) {
        this.app = app;
    }
    ...
    createPlayerCard(x, y, texture = 'backCard') {
    ...
    getValueCode(cardInfo) {
        let cardValue = cardInfo.number.toString();
        if (+cardValue >= 11) {
    ...
    updateDrawTexture(textureId) {
        this.containers['gameCards'].getChildAt(0).texture = this.textures.getById(textureId);
    }
    createDrawAndTrash() {
    ...
    revealCards(cards) {
        cards.forEach((cardInfo) => {
            const cardSprite = this.createCardWithValue(this.cardsCoords[cardInfo.cardId][0], this.cardsCoords[cardInfo.cardId][1], cardInfo.card);
...
exports.EngineRenderer = EngineRenderer;
